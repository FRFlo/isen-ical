  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (method === 'POST' && pathname === '/api/track') {
      try {
        ...
        if (!body.event || !body.event.startsWith('frontend_')) {
          return withRequestId(
            new Response(
              JSON.stringify({ error: 'Event name is invalid' }),
              {
                status: 400,
                headers: { 'Content-Type': 'application/json' },
              }
        ...
        return withRequestId(
          new Response(
            JSON.stringify({ error: `Tracking error: ${message}` }),
            {
              status: 500,
              headers: { 'Content-Type': 'application/json' },
            }
    ...
    if (method === 'POST' && pathname === '/api/generate-token') {
      ...
      try {
        ...
        const aurionService = new AurionService(env, (event, properties) => {
        ...
        track('token_generation_succeeded', {
    ...
    try {
      ...
      if (
        ...
        track('calendar_basic_auth_failed', {
          reason: 'invalid_credentials',
        }, userDistinctId);
        return withRequestId(
          new Response('Invalid credentials', {
